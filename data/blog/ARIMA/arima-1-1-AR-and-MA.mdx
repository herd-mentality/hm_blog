---
title: ARIMA Modelling In-depth Dive: 1.1 AR and MA 
date: '2023-10-29'
tags: ['ARIMA', 'linear regression', 'modelling', 'time series', 'maths', 'graph', 'R',]
draft: false
summary: 'Buidling the AR and MA parts of ARIMA model mathematically'
authors: ['vincentdoan']
---
...
>[!info] Do we actually need to use all the lagged values to describe an observation? 
...
>[!info] The Auto-Regressive (AR) part of ARIMA: 
> $X_t = (\sum_{i=1}^{p} \alpha_i L^i)X_{t} \hspace{1cm} (1)$
>  where:
...
> [!info] The Moving Average (MA) part of the ARIMA model:
$X_t = \epsilon_t + (\sum_{i=1}^{q} \theta_q L^q)\epsilon_{t} \hspace{1cm} (2)$
> where:
$\theta_i$ are the parameters/ coefficients of the linear combination of the error terms
$\epsilon_t$ are the error terms at time $t$
...
>[!info] Combining the AR and MA parts, the model $ARIMA(p,q)$ is given as: 
>$(1 - \sum_{i=1}^{p} \alpha_i L^i)X_{t} = (1 + \sum_{i=1}^{q} \theta_q L^q)\epsilon_{t}$
>where:
...
> [!info] Why do we need both an AR and an MA component?
