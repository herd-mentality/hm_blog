...
On 15 June 2026 the [DTA](https://www.dta.gov.au/articles/trustworthy-ai-agents-new-technical-guidance) made its first agentic AI requirement mandatory: any APS agency running an agentic system needs a completed [AI Impact Assessment](https://www.digital.gov.au/ai/impact-assessment-tool/introduction) before it goes live. The rest of the technical and operational controls become mandatory in December 2026.

The [addendum](https://www.digital.gov.au/policy/ai/agentic-ai-addendum-background) sorts human oversight into three modes:
...
> **Building a HOTL interface?** [Agent Tower](https://www.herdmentality.xyz/blog/agent-tower-notion-hitl-interface) walks through a working example: a Notion kanban board as the human-oversight cockpit for a running agent pipeline. Its "Steer" pattern (agent posts findings, waits on a timeout for an operator note, auto-advances if none) is a HOTL-style watch-and-override loop; the post also covers a stricter "Gate" pattern for mandatory sign-off, closer to HITL.
...
> **Want claim-level grounding instead of one pass/fail score?** [Grounding Inspector](https://www.herdmentality.xyz/blog/grounding-inspector) decomposes a response into individual claims and checks each one against the source document separately, rather than a single grounding score for the whole response. Same underlying question (is this grounded in the source?), a finer-grained answer.
...
AgentCore Policy (GA 3 March 2026, Guardrails support added 17 June 2026) runs on [Cedar](https://aws.amazon.com/about-aws/whats-new/2023/05/cedar-open-source-language-access-control) under the hood: you write policies in natural language and AWS converts them to Cedar, its open-source policy language. Enforcement happens at the AgentCore Gateway boundary, before a tool call completes.
