...
I'm building a Bedrock-backed application on top of [lex-au-search](https://github.com/cchew/lex-au-search), a public semantic search stack over Commonwealth legislation. The application answers practitioner questions that touch sensitive personal information: tax file numbers, estate details, Centrelink eligibility.

Before writing any production code, I wanted to know exactly what the Digital Transformation Agency's new agentic AI rules require, and exactly which Bedrock setting satisfies which requirement. I couldn't find that mapping written down anywhere, so I built the thing and tested it against a live guardrail to find out.

This post is that mapping. Every JSON block below is real output from an actual Bedrock guardrail I ran in Sydney (`ap-southeast-2`), not documentation. Two of the things I expected to find didn't hold up under test, which turned out to be the more interesting part.
...
Both runs got blocked identically, three times each, no variance:
...
Turns out Classic-tier prompt-attack scanning evaluates the whole multi-block content array regardless of tags. The qualifier system looks like it's there to scope PII, topic and grounding checks, not prompt-attack detection. That's good news for anyone building a RAG pipeline on Bedrock: the untagged-corpus bypass I was testing for doesn't currently exist. It's also a reminder that documentation implying a scoping behaviour is worth testing before you rely on it, because it doesn't always mean what it looks like it means.

One side finding from the same test: `ndis-eligibility` fired on content that mentions neither NDIS nor eligibility, and a separate negative-control query ("What is the Centrelink income threshold for Age Pension?", which requests no account access) got blocked by the `centrelink-account` topic 3 out of 3 runs. More on that in the denied topics section below.
...
I confirmed the gap directly: ran a marked `ApplyGuardrail` call with logging active, and it never showed up in that log group. Only the `Converse`/`InvokeModel` path does. Worth knowing too: denied or IAM-blocked `Converse` calls get logged as well, with an `errorCode` field and no `output` block, so the log group captures attempted-but-blocked calls, not just successful ones.
...
A plain negative-control query, "What is the Centrelink income threshold for Age Pension?", which asks for no account access at all, got blocked by the `centrelink-account` topic on 3 out of 3 runs. The same test session saw `ndis-eligibility` fire on content that mentions neither NDIS nor eligibility (the corpus-injection test content from the prompt injection section above).
...
One integration trap: `ApplyGuardrail` needs three separate content blocks to score grounding, the source (`grounding_source`), the question (`query`) and the answer being scored as its own untagged block. Passing only source and query, which is a natural first read of the qualifier names, silently evaluates nothing.
...
lex-au-search is public on [GitHub](https://github.com/cchew/lex-au-search). The next post covers what an AI Impact Assessment actually looks like once you fill it out against this architecture.
