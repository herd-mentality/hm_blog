...
We’ve seen in a [previous post](https://www.herdmentality.xyz/blog/bq-ext-tbl-in-r) how to authenticate with Google Cloud Platform (GCP) using service account credentials. There are a few other ways to authenticate with the platform: 
...
This method makes use of the `gargle` R package to fetch an authentication token with the correct [OAuth 2.0](https://aaronparecki.com/oauth-2-simplified/) scopes. This requires you to run the interactive process at least once, and you may need to run it again when your token expires.

- First, load `gargle`, then run `gargle::token_fetch()` with the scopes you’d like access to. Since I’ll mainly be working with BigQuery and Cloud Storage, I’ll pass in the scopes relevant to those services. [See the full list of GCP OAuth 2.0 scopes here](https://developers.google.com/identity/protocols/oauth2/scopes#bigquery).
    - In the back-end, `gargle::token_fetch()` will run a list of credential functions (checking for existing OAuth 2.0 tokens, service/external account credentials, Application Default Credentials etc.) in sequence to first check if you have any existing authentication tokens. If it does not find any, it will run `gargle::credentials_user_oauth2()` which will kick off the process you’re about to see here. For more information on this, [see this article](https://gargle.r-lib.org/articles/how-gargle-gets-tokens.html).
...
- At this stage the page will either return `”Authentication complete. Please close this page and return to R.”`, or will provide you with an authorization code you can then copy and paste back into your R console. Once this is complete, the interactive process is done.
- You can then start using your `bigrquery`/`googleCloudStorageR` functions as normal, although they may prompt you as to whether you’d like to use an existing token for authentication.
    - To exercise more control over when this happens, we can call `bigrquery::bq_auth()` and `googleCloudStorageR::gcs_auth()` early in the process and pass our token object to them:
...
Note, however, that the first authentication call of the session will usually result in a manual prompt, asking if you want to run a fresh interactive process or to use an already cached result. If we know that we won’t want to log into any other accounts, we can automate this by finding our cached credentials and explicitly passing them to `gargle::token_fetch()`. We’ll do this in the last few sections of this article.

If this is a project only you are working on, we can do this even more simply by passing an email to our auth functions, e.g. `gargle::token_fetch(email = ‘...’)`; `bigrquery::bq_auth(email = ‘...’)`. Otherwise we’ll need a programmatic way to determine the user’s email address, which is only feasible if all user’s email addresses follow a similar pattern. More on this later on in the article.
...
3. Run the following command, which will provide you with a link. Copy this link into a browser, follow the prompts, then copy the provided authorisation code back into the terminal and press Enter.
...
```r
ddc_path <- “~/.config/gcloud/application_default_credentials.json”
...
I’m creating a library of BigQuery/Cloud Storage-centric functions that will be used by a limited set of users. I want authentication with these platforms to be as simple and hands-off as possible for these users so that they can focus on the work that really matters instead of having to juggle authentication tokens.

With this in mind, I’ve created a function that leverages the power of `gargle::token_fetch()`, but aims to keep things as automated as possible. This means that the function will:

1. First check for ADCs; if valid ADCs are found then it will return this as a token
2. Next, it will check for cached tokens. If any are found, it will filter for tokens that have the required scopes and have an email address that matches the corporate email domain name.
    ...
    - This is the additional logic that sets this apart from a normal call to `gargle::token_fetch()`. If you don’t need this, it’s likely that `token_fetch(email = ‘...’)` alone will be sufficient.
...
<details>
    ...
    #'
    #' A stand-in for now until we get a service account and can auth via JSON
    #'
    ...
    #' tokens and will use the one with the '@health.gov.au' email address by
    #' default.
    #'
    ...
    if (!silent) log_info('No ADCs found, generate in Terminal with `gcloud auth application-default login --no-launch-browser`')
    if (!silent) log_info('Falling back on cached tokens stored in ~/.cache/gargle (if any)')
    
    # If force is true, force creation of another token/authenticating with a
    # different cached one
    if (force) {
        token <- gargle::token_fetch(scopes, ...)
        return (token)
    }
    ...
    valid_cached <- sitrep %>%
        ...
        str_detect(scopes, 'bigquery') & 
            ...
            str_detect(email, 'health.gov.au')
        ) %>% 
        pull(email)
    ...
    #' Connect to Impala
    connect_impala <- function(dsn = 'Discovery Impala', ...) {
    
    # Set environment variable to access connection details
    ...
    json    = NULL,
    project = 'prj-iedap-poa', 
    dataset = 'business_poa', 
    billing = NULL,
    ...
    ) {
