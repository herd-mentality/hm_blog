...
Point a model at an Act and most of its conditions will compile to something executable. The provisions that will not are the interesting output. ClauseKit is an experiment in finding where that line falls, run across five bodies of law from the EU AI Act to Australian social security.

The pipeline turns provisions into [JSON Logic](https://jsonlogic.com/) conditions over a typed fact schema, keeps a reference from each rule back to its source provision, and evaluates the rules against facts you enter. Of the 262 rules it extracted across the five domains, 171 never reach the engine: the provision is a judgment call, not a mechanical test. That number, and what changes when the model reads structured input instead of plain text, are what the experiment is about.
