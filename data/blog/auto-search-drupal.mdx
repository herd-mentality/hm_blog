---
title  : 'ONNX-Native Fine-Tuned Semantic Search in Drupal'
date   : '2026-08-27'
...
This post covers the PHP port: what it took to run ONNX inference in-process via FFI, and the GovCMS hosting trade-offs that come with it. For the fine-tuning details and model performance, see [Auto Search: Semantic Search Inside the JVM](https://www.herdmentality.xyz/blog/auto-search).
...
- [AI Search](https://www.drupal.org/project/ai_search) with an Ollama provider: a second service running alongside Drupal, 5 to 15 GB of RAM for a typical embedding model.
...
- [Search API Embeddings](https://www.drupal.org/project/search_api_embeddings): genuinely in-process, but built on Word2Vec (2013).

One new entrant has appeared since this project was scoped. [Scolta](https://www.tag1.com/scolta/) builds a static lexical index at publish time with [Pagefind](https://pagefind.app/) and runs it entirely client-side in the browser, with an AI layer for query expansion and summaries. It is the closest thing to a "no search server" pitch, but there is no vector math and no model: it rewrites the query with an LLM and searches a lexical index, not a semantic one.
